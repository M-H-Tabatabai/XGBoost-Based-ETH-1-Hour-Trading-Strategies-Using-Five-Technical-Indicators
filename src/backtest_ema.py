# File: src/backtest_ema.py
# This script loads the trained EMA XGBoost model and evaluates it on the test set.
# It also computes the rule-based EMA baseline and saves the results.
...
model.load_model(os.path.join(MODEL_DIR, 'xgboost_ema.json'))

with open(os.path.join(MODEL_DIR, 'ema_metadata.json'), 'r') as f:
...
# -------------------------------
# 5. XGBoost EMA Strategy on test set
# -------------------------------
...
print("\n--- Rule-Based EMA (Close > EMA20) (Test) ---")
...
# -------------------------------
results = {
    'Strategy': ['XGBoost EMA', 'Rule-based EMA', 'Buy & Hold'],
...
results_df.to_csv(os.path.join(RESULT_DIR, 'ema_results.csv'), index=False)
print(f"\nResults saved to {os.path.join(RESULT_DIR, 'ema_results.csv')}")
