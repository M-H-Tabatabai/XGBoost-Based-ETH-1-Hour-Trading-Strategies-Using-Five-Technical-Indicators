# File: src/long_short/train_model_ema_short.py
# This script trains an XGBoost classifier for EMA with long/short signals,
...
# -------------------------------
train_df = pd.read_csv(os.path.join(DATA_DIR, 'train_ema_features.csv'), index_col=0, parse_dates=True)
val_df = pd.read_csv(os.path.join(DATA_DIR, 'validation_ema_features.csv'), index_col=0, parse_dates=True)
...
# -------------------------------
final_model.save_model(os.path.join(MODEL_DIR, 'xgboost_ema_short.json'))
...
with open(os.path.join(MODEL_DIR, 'ema_short_metadata.json'), 'w') as f:
    json.dump(metadata, f, indent=2)

print(f"\nModel and metadata saved to {MODEL_DIR}/")
print("EMA short training complete.")