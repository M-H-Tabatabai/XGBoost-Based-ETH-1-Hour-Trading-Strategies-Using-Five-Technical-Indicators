# File: src/long_short/train_model_volume_short.py
# This script trains an XGBoost classifier for Volume with long/short signals,
# tunes hyperparameters and two thresholds using the validation set,
# and saves the model to models/long_short/.
...
# -------------------------------
train_df = pd.read_csv(os.path.join(DATA_DIR, 'train_volume_features.csv'), index_col=0, parse_dates=True)
val_df = pd.read_csv(os.path.join(DATA_DIR, 'validation_volume_features.csv'), index_col=0, parse_dates=True)
...
# -------------------------------
final_model.save_model(os.path.join(MODEL_DIR, 'xgboost_volume_short.json'))
...
with open(os.path.join(MODEL_DIR, 'volume_short_metadata.json'), 'w') as f:
    json.dump(metadata, f, indent=2)

print(f"\nModel and metadata saved to {MODEL_DIR}/")
print("Volume short training complete.")