# File: src/long_short/backtest_sma_short.py
# This script loads the trained SMA short-enabled model, generates long/short signals,
...
model.load_model(os.path.join(MODEL_DIR, 'xgboost_sma_short.json'))

with open(os.path.join(MODEL_DIR, 'sma_short_metadata.json'), 'r') as f:
...
print("\n--- XGBoost SMA Long/Short (Test) ---")
print(f"Total Return: {total_return_xgb:.2f}%")
print(f"Max Drawdown: {max_dd_xgb:.2f}%")
print(f"Sharpe Ratio: {sharpe_xgb:.2f}")

# -------------------------------
# 6. Rule-based SMA long/short baseline
# -------------------------------
# Long if close > SMA20, short if close < SMA20
test_df['signal_rule'] = np.where(test_df['close'] > test_df['SMA_20'], 1, -1)
...
print("\n--- Rule-Based SMA Long/Short (Test) ---")
...
# -------------------------------
results = {
    'Strategy': ['XGBoost SMA Long/Short', 'Rule-based SMA Long/Short', 'Buy & Hold'],
...
results_df.to_csv(os.path.join(RESULT_DIR, 'sma_results_short.csv'), index=False)
print(f"\nResults saved to {os.path.join(RESULT_DIR, 'sma_results_short.csv')}")
print("SMA short backtest complete.")