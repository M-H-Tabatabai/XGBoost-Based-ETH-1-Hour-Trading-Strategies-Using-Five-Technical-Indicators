# File: src/long_short/backtest_rsi_short.py
# This script loads the trained RSI short-enabled model, generates long/short signals,
...
model.load_model(os.path.join(MODEL_DIR, 'xgboost_rsi_short.json'))

with open(os.path.join(MODEL_DIR, 'rsi_short_metadata.json'), 'r') as f:
...
print("\n--- XGBoost RSI Long/Short (Test) ---")
print(f"Total Return: {total_return_xgb:.2f}%")
print(f"Max Drawdown: {max_dd_xgb:.2f}%")
print(f"Sharpe Ratio: {sharpe_xgb:.2f}")

# -------------------------------
# 6. Rule-based RSI long/short baseline
# -------------------------------
# Stateful: long if RSI < 30, short if RSI > 70, flat otherwise.
...
for i in range(len(test_df)):
    ...
    if in_position == 0:
        ...
        if rsi_val < 30:
            in_position = 1
    position_rule[i] = in_position

signal_rule = pd.Series(position_rule, index=test_df.index)
equity_rule, net_ret_rule = backtest_long_short(signal_rule, test_df['open'], fee=0.001)
...
print("\n--- Rule-Based RSI Long/Short (Test) ---")
...
# -------------------------------
results = {
    'Strategy': ['XGBoost RSI Long/Short', 'Rule-based RSI Long/Short', 'Buy & Hold'],
...
results_df.to_csv(os.path.join(RESULT_DIR, 'rsi_results_short.csv'), index=False)
print(f"\nResults saved to {os.path.join(RESULT_DIR, 'rsi_results_short.csv')}")
print("RSI short backtest complete.")