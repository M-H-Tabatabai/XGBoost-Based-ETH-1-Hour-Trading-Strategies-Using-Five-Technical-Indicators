# File: src/long_short/train_model_sma_short.py
# This script trains an XGBoost classifier for SMA with long/short signals,
...
def backtest_long_short(signal_series, open_prices, fee=0.001):
    ...
    # Return from holding position from open_t to open_{t+1}
    next_open = open_prices.shift(-1)
    # For long: ret = next/open - 1 ; for short: ret = open/next - 1
...
# Search grid for upper and lower thresholds
...
for upper in upper_grid:
    for lower in lower_grid:
        if lower >= upper:
            continue
        # Generate signals: long if prob > upper, short if prob < lower, else flat
...
# -------------------------------
final_model.save_model(os.path.join(MODEL_DIR, 'xgboost_sma_short.json'))
...
with open(os.path.join(MODEL_DIR, 'sma_short_metadata.json'), 'w') as f:
    json.dump(metadata, f, indent=2)

print(f"\nModel and metadata saved to {MODEL_DIR}/")
print("SMA short training complete.")