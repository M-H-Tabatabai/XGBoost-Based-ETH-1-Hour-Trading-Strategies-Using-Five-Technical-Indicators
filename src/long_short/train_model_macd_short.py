# File: src/long_short/train_model_macd_short.py
# This script trains an XGBoost classifier for MACD with long/short signals,
...
# -------------------------------
train_df = pd.read_csv(os.path.join(DATA_DIR, 'train_macd_features.csv'), index_col=0, parse_dates=True)
val_df = pd.read_csv(os.path.join(DATA_DIR, 'validation_macd_features.csv'), index_col=0, parse_dates=True)
...
# -------------------------------
final_model.save_model(os.path.join(MODEL_DIR, 'xgboost_macd_short.json'))
...
with open(os.path.join(MODEL_DIR, 'macd_short_metadata.json'), 'w') as f:
    json.dump(metadata, f, indent=2)

print(f"\nModel and metadata saved to {MODEL_DIR}/")
print("MACD short training complete.")