# File: src/long_short/backtest_macd_short.py
# This script loads the trained MACD short-enabled model, generates long/short signals,
...
model.load_model(os.path.join(MODEL_DIR, 'xgboost_macd_short.json'))

with open(os.path.join(MODEL_DIR, 'macd_short_metadata.json'), 'r') as f:
...
test_df['MACD_line'] = test_df['EMA_12'] - test_df['EMA_26']
test_df['MACD_signal'] = test_df['MACD_line'].ewm(span=9, adjust=False).mean()
test_df['signal_rule'] = np.where(test_df['MACD_line'] > test_df['MACD_signal'], 1, -1)
...
print("\n--- Rule-Based MACD Long/Short (Test) ---")
...
# -------------------------------
results = {
    'Strategy': ['XGBoost MACD Long/Short', 'Rule-based MACD Long/Short', 'Buy & Hold'],
...
results_df.to_csv(os.path.join(RESULT_DIR, 'macd_results_short.csv'), index=False)
print(f"\nResults saved to {os.path.join(RESULT_DIR, 'macd_results_short.csv')}")
print("MACD short backtest complete.")