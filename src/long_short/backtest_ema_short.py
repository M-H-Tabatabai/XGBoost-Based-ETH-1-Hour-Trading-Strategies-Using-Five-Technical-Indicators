# File: src/long_short/backtest_ema_short.py
# This script loads the trained EMA short-enabled model, generates long/short signals,
...
# -------------------------------
test_df = pd.read_csv(os.path.join(DATA_DIR, 'test_ema_features.csv'), index_col=0, parse_dates=True)
...
model.load_model(os.path.join(MODEL_DIR, 'xgboost_ema_short.json'))

with open(os.path.join(MODEL_DIR, 'ema_short_metadata.json'), 'r') as f:
...
print("\n--- Rule-Based EMA Long/Short (Test) ---")
...
# -------------------------------
results = {
    'Strategy': ['XGBoost EMA Long/Short', 'Rule-based EMA Long/Short', 'Buy & Hold'],
...
results_df.to_csv(os.path.join(RESULT_DIR, 'ema_results_short.csv'), index=False)
print(f"\nResults saved to {os.path.join(RESULT_DIR, 'ema_results_short.csv')}")
print("EMA short backtest complete.")