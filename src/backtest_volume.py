# File: src/backtest_volume.py
# This script loads the trained Volume XGBoost model and evaluates it on the test set.
# It also computes the rule-based Volume baseline and saves the results.
...
model.load_model(os.path.join(MODEL_DIR, 'xgboost_volume.json'))

with open(os.path.join(MODEL_DIR, 'volume_metadata.json'), 'r') as f:
...
# -------------------------------
# 5. XGBoost Volume Strategy on test set
# -------------------------------
...
print("\n--- XGBoost Volume Strategy (Test) ---")
print(f"Total Return: {total_return_xgb:.2f}%")
print(f"Max Drawdown: {max_dd_xgb:.2f}%")
print(f"Sharpe Ratio: {sharpe_xgb:.2f}")

# -------------------------------
# 6. Rule-based Volume Baseline
# -------------------------------
# Rule: long when volume > 1.5 * volume_SMA_20 AND close > open
condition = (test_df['volume'] > 1.5 * test_df['volume_SMA_20']) & (test_df['close'] > test_df['open'])
test_df['signal_rule'] = condition.astype(int)
...
# -------------------------------
results = {
    'Strategy': ['XGBoost Volume', 'Rule-based Volume', 'Buy & Hold'],
...
results_df.to_csv(os.path.join(RESULT_DIR, 'volume_results.csv'), index=False)
print(f"\nResults saved to {os.path.join(RESULT_DIR, 'volume_results.csv')}")
