# File: src/backtest_rsi.py
# This script loads the trained RSI XGBoost model and evaluates it on the test set.
# It also computes the rule-based RSI baseline and saves the results.
...
# -------------------------------
test_df = pd.read_csv(os.path.join(DATA_DIR, 'test_rsi_features.csv'), index_col=0, parse_dates=True)
...
model.load_model(os.path.join(MODEL_DIR, 'xgboost_rsi.json'))

with open(os.path.join(MODEL_DIR, 'rsi_metadata.json'), 'r') as f:
...
# -------------------------------
# 5. XGBoost RSI Strategy on test set
# -------------------------------
...
print("\n--- XGBoost RSI Strategy (Test) ---")
print(f"Total Return: {total_return_xgb:.2f}%")
print(f"Max Drawdown: {max_dd_xgb:.2f}%")
print(f"Sharpe Ratio: {sharpe_xgb:.2f}")

# -------------------------------
# 6. Rule-based RSI Baseline
# -------------------------------
# Rule: enter long when RSI_14 < 30, exit/flat when RSI_14 > 70, hold between.
# This is stateful: need to carry forward position based on crossing events.
rsi_series = test_df['RSI_14']

# Initialize position array with zeros
position_rule = np.zeros(len(test_df), dtype=int)
in_position = False
...
signal_rule = pd.Series(position_rule, index=test_df.index)
equity_rule, net_ret_rule = backtest_accurate(signal_rule, test_df['open'], test_df['close'], fee=0.001)
...
# -------------------------------
results = {
    'Strategy': ['XGBoost RSI', 'Rule-based RSI', 'Buy & Hold'],
...
results_df.to_csv(os.path.join(RESULT_DIR, 'rsi_results.csv'), index=False)
print(f"\nResults saved to {os.path.join(RESULT_DIR, 'rsi_results.csv')}")
